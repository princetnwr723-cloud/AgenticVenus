...
// components/ComputerViewPanel.tsx
// Shows the live VNC stream of the agent's cloud computer, so the user
// can watch exactly what it's doing in real time.
      ...
      <div className="animate-scale-in flex h-full w-full max-w-2xl flex-col bg-[#1e1c19] text-cream shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-2">
            {streamUrl ? (
              ...
              <button
                onClick={onStart}
                disabled={starting}
                className="rounded-md bg-clay px-2.5 py-1 text-xs font-medium text-cream hover:bg-clay-dark disabled:opacity-50"
              >
        ...
        {streamUrl ? (
          ...
          <div className="flex flex-1 items-center justify-center px-8 text-center text-sm text-cream/40">
            {starting
              ? "Spinning up a 4 vCPU / 16GB RAM cloud desktop..."
              : "Start a cloud computer to give the agent a real desktop it can see and use."}
          </div>
        )}
