      ...
      <div className="flex h-full w-full max-w-5xl flex-col bg-[#171614] text-cream shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div>
            ...
            <div className="mt-1 text-[11px] text-cream/40">Persistent Daytona workspace · {status}</div>
          </div>
          <button onClick={onClose} className="rounded border border-white/10 px-3 py-1.5 text-xs text-cream/60 hover:text-cream">Close</button>
        </header>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[1fr_1fr]">
          <section className="flex min-h-0 flex-col border-b border-white/10 lg:border-b-0 lg:border-r">
            <div className="border-b border-white/10 px-4 py-3 text-xs text-cream/50">Terminal</div>
            ...
            <form onSubmit={execute} className="flex border-t border-white/10 bg-[#11110f] p-3">
              ...
              <input value={command} onChange={(e) => setCommand(e.target.value)} placeholder="npm install, npm run build, ls -la..." className="min-w-0 flex-1 bg-transparent font-mono text-xs text-cream outline-none" autoComplete="off" />
